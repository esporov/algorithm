...
public class Main3 {
    ...
    public static int help(int myTeam, int barack, int opponentTeam, int[] tempRound) {
        ...
        if (tempOp == 0 && tempBar == 0) {// 250 - 150 = 100
            return 1;
        }
        if (tempMy > tempOp - tempMy && tempBar == 0) {
            tempRound[0]++;
            return help(tempMy, tempBar, tempOp, tempRound);
        } else return -1;
    }
    ...
    public static int attack(int myTeam, int[] barack, int[] opponentTeam, int reinforcement, int round) {
        if (barack[0] != 0) {
            ...
            int[] tempRound = new int[]{round};
            int tempHelp = help(myTeam, barack[0], opponentTeam[0], tempRound);
            if (tempHelp == 1) {
                flag[0] = true;
            }
            if (tempHelp == 1 || !flag[0]) {
